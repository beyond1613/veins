...
void TraCIScenarioManager::init_traci() {
    {
        ...
        if ((apiVersion == 8)) {
            ...
            error(
                    "TraCI server \"%s\" reports API version %d, which is unsupported. We recommend using SUMO 0.21.0.",
...

    {
        ...
        MYDEBUG << "TraCI reports network boundaries (" << x1 << ", " << y1
                       << ")-(" << x2 << ", " << y2 << ")" << endl;
        if ((traci2omnet(netbounds2).x > world->getPgs()->x)
                || (traci2omnet(netbounds1).y > world->getPgs()->y))
            MYDEBUG << "WARNING: Playground size (" << world->getPgs()->x
                           << ", " << world->getPgs()->y
                           << ") might be too small for vehicle at network bounds ("
                           << traci2omnet(netbounds2).x << ", "
                           << traci2omnet(netbounds1).y << ")" << endl;
    }
    ...
    if (obstacles) {
        {
            ...
            for (std::list<std::string>::iterator i = ids.begin();
                    i != ids.end(); ++i) {
                std::string id = *i;
                std::string typeId = getCommandInterface()->getPolygonTypeId(
                        id);
                if (typeId == "building") {
                    std::list<TraCICoord> coords =
                            getCommandInterface()->getPolygonShape(id);
...
Coord TraCIScenarioManager::traci2omnet(TraCICoord coord) const {
    return Coord(coord.x - netbounds1.x + margin,
            (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y) + margin);
}

std::list<Coord> TraCIScenarioManager::traci2omnet(
        const std::list<TraCICoord>& list) const {
...
std::list<TraCICoord> TraCIScenarioManager::omnet2traci(
        const std::list<Coord>& list) const {
    std::list<TraCICoord> result;
    std::transform(list.begin(), list.end(), std::back_inserter(result),
            std::bind1st(
                    std::mem_fun<TraCICoord, TraCIScenarioManager, Coord>(
                            &TraCIScenarioManager::omnet2traci), this));
    return result;
}
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId,
    ...
    if (!inRoi) {
        if (mod) {
            ...
            /*
             *  Description : delete module for objectId2 as well
             *  Date : 2015-10-13
             *  Author : Will Tseng
             */
