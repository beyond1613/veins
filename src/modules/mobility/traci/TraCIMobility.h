...
public:
    ...
    TraCIMobility() :
            BaseMobility(), isPreInitialized(false) {
    }
    ...
    void commandChangeRoute(std::string roadId, double travelTime) {
        ...
        /*
         * Description : ignore command called by mod2
         * Date : 2015-10-13
         * Author : Will Tseng
         */
    ...
    double commandDistanceRequest(Coord position1, Coord position2,
            bool returnDrivingDistance) {
        return getCommandInterface()->distanceRequest(
                getManager()->omnet2traci(position1),
                getManager()->omnet2traci(position2), returnDrivingDistance);
    }
    void commandStopNode(std::string roadId, double pos, uint8_t laneid,
            double radius, double waittime) {
        getCommandInterface()->stopNode(getExternalId(), roadId, pos, laneid,
                radius, waittime);
    }
    ...
    std::list<Coord> commandGetPolygonShape(std::string polyId) {
        return getManager()->traci2omnet(
                getCommandInterface()->getPolygonShape(polyId));
    }
    void commandSetPolygonShape(std::string polyId, std::list<Coord> points) {
        getCommandInterface()->setPolygonShape(polyId,
                getManager()->omnet2traci(points));
    }
    bool commandAddVehicle(std::string vehicleId, std::string vehicleTypeId,
            std::string routeId, simtime_t emitTime_st =
                    -TraCICommandInterface::DEPART_NOW, double emitPosition =
                    -TraCICommandInterface::DEPART_POS_BASE, double emitSpeed =
                    -TraCICommandInterface::DEPART_SPEED_MAX, int8_t emitLane =
                    -TraCICommandInterface::DEPART_LANE_BEST_FREE) {
        return getCommandInterface()->addVehicle(vehicleId, vehicleTypeId,
