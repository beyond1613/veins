...
simtime_t BaseDecider::processNewSignal(AirFrame* frame) {
    ...
    deciderEV << "Signal is strong enough (" << recvPower << " > "
                     << sensitivity << ") -> Trying to receive AirFrame."
                     << endl;
...
simtime_t BaseDecider::handleChannelSenseRequest(ChannelSenseRequest* request) {
    ...
    if (currentChannelSenseRequest.first != request) {
        opp_error(
                "Got a new ChannelSenseRequest while already handling another one!");
        return notAgain;
    }
...
simtime_t BaseDecider::handleNewSenseRequest(ChannelSenseRequest* request) {
    ...
    currentChannelSenseRequest.canAnswerAt = canAnswerCSR(
...
void BaseDecider::channelStateChanged() {
    ...
    if (canAnswerAt != currentChannelSenseRequest.canAnswerAt) {
        ...
        if (canAnswerAt == phy->getSimTime()) {
            phy->cancelScheduledMessage(
                    currentChannelSenseRequest.getRequest());
            answerCSR(currentChannelSenseRequest);
        } else {
...
double BaseDecider::calcChannelSenseRSSI(simtime_t_cref start,
        simtime_t_cref end) {
    ...
    Mapping::argument_value_t rssi = MappingUtils::findMax(*rssiMap,
...
void BaseDecider::answerCSR(CSRInfo& requestInfo) {
    ...
    double rssiValue = calcChannelSenseRSSI(requestInfo.second,
            phy->getSimTime());
...
Mapping* BaseDecider::calculateSnrMapping(AirFrame* frame) {
    ...
    Mapping* snrMap = MappingUtils::divide(*recvPowerMap, *noiseMap,
            Argument::MappedZero);
...
Mapping* BaseDecider::calculateRSSIMapping(simtime_t_cref start,
    ...
    if (exclude)
        deciderEV << "Creating RSSI map excluding AirFrame with id "
                         << exclude->getId() << endl;
    else
...
double BaseDecider::calculateSinr(simtime_t_cref start, simtime_t_cref end,
        AirFrame* exclude) {
    double sinr = exclude->getSignal().getRecvPower() - sensitivity;

    if (exclude)
        deciderEV << "CalculateSinr excluding AirFrame with id "
                         << exclude->getId() << endl;
    else
    ...
    for (AirFrameVector::const_iterator it = airFrames.begin();
        ...
        deciderEV << "Considering Interference by Airframe with ID "
                         << (*it)->getId() << ". Starts at "
