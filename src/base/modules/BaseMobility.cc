...
const simsignalwrap_t BaseMobility::mobilityStateChangedSignal =
        simsignalwrap_t(MIXIM_SIGNAL_MOBILITY_CHANGE_NAME);

namespace {
bool isInBoundary(Coord c, Coord lowerBound, Coord upperBound) {
    return lowerBound.x <= c.x && c.x <= upperBound.x && lowerBound.y <= c.y
            && c.y <= upperBound.y && lowerBound.z <= c.z && c.z <= upperBound.z;
}

}

BaseMobility::BaseMobility() :
        BatteryAccess(), move(), playgroundScaleX(1), playgroundScaleY(1), origDisplayWidth(
                0), origDisplayHeight(0), origIconSize(0) {
}

BaseMobility::BaseMobility(unsigned stacksize) :
        BatteryAccess(stacksize), move(), playgroundScaleX(1), playgroundScaleY(
                1), origDisplayWidth(0), origDisplayHeight(0), origIconSize(0) {
}
...
void BaseMobility::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        if (move.getSpeed() > 0 && updateInterval > 0) {
            coreEV << "Host is moving, speed=" << move.getSpeed()
                          << " updateInterval=" << updateInterval << endl;
    ...
    if (ev.isGUI()) {
        ...
        if (!world->use2D() && scaleNodeByDepth) {
            ...
            double depthScale = minScale
                    + (maxScale - minScale)
                            * (1.0 - move.getStartPos().z / playgroundSizeZ());
...
void BaseMobility::reflectIfOutside(BorderHandling wo, Coord& stepTarget,
        Coord& targetPos, Coord& step, double& angle) {
...
void BaseMobility::wrapIfOutside(BorderHandling wo, Coord& stepTarget,
        Coord& targetPos) {
    switch (wo) {
    case X_SMALLER:
    case X_BIGGER:
...
BaseMobility::BorderHandling BaseMobility::checkIfOutside(Coord targetPos,
    ...
    coreEV << "checkIfOutside, outside=" << outside << " borderStep: "
                  << borderStep.info() << endl;
...
void BaseMobility::goToBorder(BorderPolicy policy, BorderHandling wo,
    ...
    coreEV << "goToBorder: startPos: " << move.getStartPos().info()
                  << " borderStep: " << borderStep.info() << " BorderPolicy: "
                  << policy << " BorderHandling: " << wo << endl;
    ...
    coreEV << "goToBorder: startPos: " << move.getStartPos().info()
                  << " borderStep: " << borderStep.info() << " borderStart: "
                  << borderStart.info() << " factor: " << factor << endl;
}
...
bool BaseMobility::handleIfOutside(BorderPolicy policy, Coord& stepTarget,
    ...
    coreEV << "old values: stepTarget: " << stepTarget.info() << " step: "
                  << step.info() << " targetPos: " << targetPos.info()
                  << " angle: " << angle << endl;
    ...
    coreEV << "new values: stepTarget: " << stepTarget.info() << " step: "
                  << step.info() << " angle: " << angle << endl;
    ...
    coreEV << "border handled, borderStep: " << borderStep.info()
                  << "borderStart: " << borderStart.info() << " stepTarget "
                  << stepTarget.info() << endl;
