...
public:
    Move() :
            startPos(), lastPos(0.0, 0.0, DBL_MAX), startTime(), direction(), speed(
                    0.0) {
    }
    Move(const Move& mSrc) :
            startPos(mSrc.startPos), lastPos(mSrc.lastPos), startTime(
                    mSrc.startTime), direction(mSrc.direction), speed(
                    mSrc.speed) {
    }
    ...
    void setDirectionByVector(const Coord& direction) {
        assert(
                FWMath::close(direction.squareLength(), 1.0)
    ...
    virtual Coord getPositionAt(simtime_t_cref actualTime = simTime()) const {
        ...
        return startPos
                + (direction * speed * SIMTIME_DBL(actualTime - startTime));
    }
